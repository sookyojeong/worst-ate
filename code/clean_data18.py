...
df = pd.read_csv('../data/2018/personsx18.csv')
...
df['overnight_hospital_times'] = df['hospno_p'].apply(lambda x: 0 if x in [7,8,9] else x)
df['overnight_hospital_nights'] = df['hpnite_p'].apply(lambda x: 0 if x in [97,98,99] else x)
df['care_athome_2wks'] = df['phchm2w'] == 1
df['care_athome_2wks_times'] = df['phchm2w'].apply(lambda x: 0 if x in [97,98,99] else x)
df['care_phone_2wks'] = df['phcph2wr'] == 1
df['care_phone_2wks_times'] = df['phcpn2wp'].apply(lambda x: 0 if x in [97,98,99] else x)
...
df['milcare_'] = df['militarn'].isin( [1,2])
df['milcare_tricare'] = df['milspc1'] == 1
df['milcare_va'] = df['milspc2n'] == 1
df['milcare_champ-va'] = df['milspc3'] == 1
df['milcare_other'] = df['milspc4'] == 1
df['milcare_tricare_prime'] = df['milmann'] == 1
df['milcare_tricare_standard_extra'] = df['milmann'] == 2
df['milcare_tricare_forlife'] = df['milmann'] == 4

df['ihs_'] = df['ihs']==1

df['hilast_6mo'] = df['hilast2'] == 1
df['hilast_1yr'] = df['hilast2'] == 2
df['hilast_3yr'] = df['hilast2'] == 3
df['hilast_3yrplus'] = df['hilast2'] == 4
...
df.to_csv('../data/clean/data2018.csv')