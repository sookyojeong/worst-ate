...
df = pd.read_stata('../data/personsx08.dta')
...
df['bio_mother'] = df['mom_deg'] == '1 Biological'
df['bio_father'] = df['dad_deg'] == '1 Biological'
...
df['limited_to_work_gt18'] = df['plawklim'] == '1 Yes'
...
df['retard_lt18'] = df['lahcc7'] == '1 Mentioned'
...
df['retard_1yrp_lt18'] = df['lcdurb7'] == '4 More than 1 year'
...
df['retard_chrc_lt18'] = df['lcchrc7'] == '1 Chronic'
...
df['retard_gt18'] = df['lahca14'] == '1 Mentioned'
...
df['retard_1yrp_gt18'] = df['ladurb14'] == '4 More than 1 year'
...
df['retard_chrc_gt18'] = df['lachrc14'] == '1 Chronic'
...
df['notcov'] = df['notcov'].apply(lambda x: True if x=='1 Not covered' else False)
df['medicare'] = df['medicare'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
...
df['medicare_pay_premium'] = df['mcpaypre'] == '1 Yes'
df['medicare_partD'] = df['mcpartd'] == '1 Yes'

df['medicaid'] = df['medicaid'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
df['medicaid_md'] = df['machmd'].apply(lambda x: True if x in ['1 Any doctor', '2 Select from book/list', '3 Doctor is assigned'] else False)
...
df['single'] = df['single'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
...
df['private'] = df['private'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
df['private1_hmo'] = df['hitypen1'] == '1 HMO'
df['private1_primary'] = df['whonam1'] == '1 In own name'
df['private1_thru_wrk_employer'] = df['plnwrkn1'] == '01 Through employer'
df['private1_thru_wrk_union'] = df['plnwrkn1'] == '02 Through union'
df['private1_thru_wrk_other'] = df['plnwrkn1'].apply(lambda x: True if x in ["03 Through workplace, but don't know if employer or union", '04 Through workplace, self-employed or professional association'] else False)
df['private1_thru_direct'] = df['plnwrkn1'] == '05 Purchased directly'
df['private1_thru_state'] = df['plnwrkn1'] == '06 Through a state/local government or community program'
df['private1_thru_school'] = df['plnwrkn1'] == '08 Through school'
...
df['private1_hdhp'] = df['hdhp1'] == '1 Less than [$1,100/$2,200]'
...
df['private2_hmo'] = df['hitypen2'] == '1 HMO'
df['private2_primary'] = df['whonam2'] == '1 In own name'
df['private2_thru_wrk_employer'] = df['plnwrkn2'] == '01 Through employer'
df['private2_thru_wrk_union'] = df['plnwrkn2'] == '02 Through union'
df['private2_thru_wrk_other'] = df['plnwrkn2'].apply(lambda x: True if x in ["03 Through workplace, but don't know if employer or union", '04 Through workplace, self-employed or professional association'] else False)
df['private2_thru_direct'] = df['plnwrkn2'] == '05 Purchased directly'
df['private2_thru_state'] = df['plnwrkn2'] == '06 Through a state/local government or community program'
df['private2_thru_school'] = df['plnwrkn2'] == '08 Through school'
...
df['private2_hdhp'] = df['hdhp2'] == '1 Less than [$1,100/$2,200]'
...
df['schip'] = df['schip'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
df['schip_md'] = df['stdoc1'].apply(lambda x: True if x in ['1 Any doctor', '2 Select from book/list', '3 Doctor is assigned'] else False)
#df['schip_primary_routine'] = df['stpcmd1'] == '1 Yes'
#df['schip_special_referral'] = df['stref1'] == '1 Yes'

df['othpub'] = df['othpub'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
df['othpub_md'] = df['stdoc2'].apply(lambda x: True if x in ['1 Any doctor', '2 Select from book/list', '3 Doctor is assigned'] else False)
#df['othpub_primary_routine'] = df['stpcmd2'] == '1 Yes'
#df['othpub_special_referral'] = df['stref2'] == '1 Yes'

df['othgov'] = df['othgov'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
df['othgov_md'] = df['stdoc3'].apply(lambda x: True if x in ['1 Any doctor', '2 Select from book/list', '3 Doctor is assigned'] else False)
#df['othgov_primary_routine'] = df['stpcmd3'] == '1 Yes'
#df['othgov_special_referral'] = df['stref3'] == '1 Yes'

df['milcare'] = df['milcare'].apply(lambda x: True if x in ['1 Yes, information', '2 Yes, but no information'] else False)
...
df['milcare_tricare_extra'] = df['milman'] == '2 TRICARE Extra'
df['milcare_tricare_standard'] = df['milman'] == '3 TRICARE Standard'
df['milcare_tricare_forlife'] = df['milman'] == '4 TRICARE for life'

df['ihs'] = df['ihs'].apply(lambda x: True if x=='1 Yes' else False)
...
df['fsa'] = df['fsa'].apply(lambda x: True if x=='1 Yes' else False)
...
df['educ_highschl'] = df['educ1'].apply(lambda x: True if x in ['13 GED or equivalent', '14 High School Graduate'] else False)
df['educ_somecol'] = df['educ1'].apply(lambda x: True if x in ['15 Some college, no degree', '16 Associate degree: occupational, technical, or vocational program',
                                     '17 Associate degree: academic program'] else False)
df['educ_colplus'] = df['educ1'].apply(lambda x: True if x in ["18 Bachelor's degree (Example: BA, AB, BS, BBA)",
                                                               "19 Master's degree (Example: MA, MS, MEng, MEd, MBA)",
                                                               "20 Professional School degree (Example: MD, DDS, DVM, JD)",
                                                               "21 Doctoral degree (Example: PhD, EdD)"] else False)
#df['military_honorable_discharge'] = df['pmiltry'] == '1 Yes'

df['employed']  = df['doinglwp'].apply(lambda x: True if x in ['1 Working for pay at a job or business',
                                                               '2 With a job or business but not at work'] else False)
...
df['earnings_lt5k'] = df['ernyr_p'] == '1 $01-$4,999'
df['earnings_lt10k'] = df['ernyr_p'] == '2 $5,000-$9,999'
df['earnings_lt15k']= df['ernyr_p'] == '3 $10,000-$14,999'
df['earnings_lt20k']= df['ernyr_p'] == '4 $15,000-$19,999'
df['earnings_lt25k']= df['ernyr_p'] == '5 $20,000-$24,999'
df['earnings_lt34k']= df['ernyr_p'] == '6 $25,000-$34,999'
df['earnings_lt45k']= df['ernyr_p'] == '7 $35,000-$44,999'
df['earnings_lt55k']= df['ernyr_p'] == '8 $45,000-$54,999'
df['earnings_lt65k']= df['ernyr_p'] == '9 $55,000-$64,999'
...
df['year'] = 2008
df.to_csv('../data/clean/data2008.csv')